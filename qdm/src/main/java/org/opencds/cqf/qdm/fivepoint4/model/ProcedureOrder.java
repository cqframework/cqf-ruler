...
public abstract class ProcedureOrder extends BaseType implements Serializable
{
    ...
    {
        if (other instanceof ProcedureOrder)
        {
            ProcedureOrder procedureOrder = (ProcedureOrder) other;
            super.copy(procedureOrder);
            setAuthorDatetime(procedureOrder.getAuthorDatetime());
            setReason(procedureOrder.getReason());
            setAnatomicalLocationSite(procedureOrder.getAnatomicalLocationSite());
            setOrdinality(procedureOrder.getOrdinality());
            setNegationRationale(procedureOrder.getNegationRationale());
        }
        else
        {
