...
public class ModuleConfigurationResolver {
	...
	private static final String ACTIVE_MEDICATION_ORDERS = "MedicationRequest?patient={{context.patientId}}&date=ge{{today - 13 months - 1 day}}&status=active,completed,stopped&category=community&intent=order&_include=MedicationRequest:medication&_count=10";
	private static final String ACTIVE_CATEGORIZED_CONDITIONS = "Condition?patient={{context.patientId}}&category=health-concern,problem-list-item&clinical-status=active&_count=10";
	...
	private static final String UDS_LABS_POST = "Observation?subject={{context.patientId}}&category=laboratory&date=ge{{today - 1 year}}&_count=10";
	...
	/**
	 * Resolve medication references within the provided bundle by copying the Medication.code
	 * into the MedicationRequest, for any MedicationRequest that references a Medication in
	 * that same bundle. Also returns a new bundle to avoid mutating the original.
	 */
	public Bundle resolveMedicationReferences(Bundle bundle) {
		var newBundle = new Bundle().setType(Bundle.BundleType.SEARCHSET);

		// Extract any Medication resources
		...
		for (Bundle.BundleEntryComponent entry : bundle.getEntry()) {
			...
			if (resource instanceof MedicationRequest) {
				var medReq = (MedicationRequest) resource;
				if (medReq.hasMedicationReference()) {
					var ref = medReq.getMedicationReference().getReference();
					var idPart = ref.replace("Medication/", "");
					// Find corresponding Medication
					var match = medications.stream()
						.filter(m -> m.getIdElement().getIdPart().equals(idPart))
						.findFirst();
                    // Replace the Reference with the code
                    match.ifPresent(medication -> medReq.setMedication(medication.getCode()));
				}
			}
			// Add to new bundle
...
//		String encounterDiagnosisUrl = ENCOUNTER_DIAGNOSIS_CONDITIONS
//			.replace("{{context.patientId}}", patientId);
//
//		String pastYearEncountersUrl = ENCOUNTERS_IN_PAST_YEAR
//			.replace("{{context.patientId}}", patientId)
...
//		urls.add(encounterDiagnosisUrl);
//		urls.add(pastYearEncountersUrl);
