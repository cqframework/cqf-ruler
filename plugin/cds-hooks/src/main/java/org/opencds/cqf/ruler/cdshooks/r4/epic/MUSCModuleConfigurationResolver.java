...
public class MUSCModuleConfigurationResolver {
	/**
	 * Default HTTP timeout (in milliseconds) for the prefetch client.
	 * This is a sane default and can be overridden via the overloaded constructor.
	 */
	private static final int DEFAULT_PREFETCH_TIMEOUT_MS = 5000;
	private static final String PATIENT = "Patient/{{context.patientId}}";
	private static final String ACTIVE_MEDICATION_ORDERS = "MedicationRequest?patient={{context.patientId}}&date=ge{{today() - 1 year - 1 month - 1 day}}&status=active,completed,stopped&category=community&intent=order,plan&_include=MedicationRequest:medication";
	...
	public MUSCModuleConfigurationResolver(FhirContext fhirContext, Endpoint prefetchEndpoint, CdsHooksRequest request) {
		this(fhirContext, prefetchEndpoint, request, DEFAULT_PREFETCH_TIMEOUT_MS);
	}

	public MUSCModuleConfigurationResolver(FhirContext fhirContext,
		...
		// Configure HTTP timeouts for the prefetch client
		var clientFactory = fhirContext.getRestfulClientFactory();
		if (prefetchTimeoutMs > 0) {
			clientFactory.setConnectTimeout(prefetchTimeoutMs);
			clientFactory.setSocketTimeout(prefetchTimeoutMs);
			clientFactory.setConnectionRequestTimeout(prefetchTimeoutMs);
		}
	...
	private Bundle searchWithPagination(String resourceType, Map<String, List<String>> queryMap, MCLQueryResult queryResult) {
		...
		var result = search.returnBundle(Bundle.class).usingStyle(SearchStyleEnum.POST).execute();
