package org.opencds.cqf.ruler.plugin.devtools.r4;
 ...
 * implementation
 * to enable {@link ValueSet ValueSet} expansion and validation without complete
 * {@link CodeSystem CodeSystems}.
 */
public class CodeSystemUpdateProvider implements OperationProvider, IdUtilities {
	...
	public OperationOutcome updateCodeSystems() {
		IBundleProvider valuesets = this.myValueSetDaoR4.search(SearchParameterMap.newSynchronous());
	...
	private CodeSystem getCodeSystemByUrl(String url) {
		IBundleProvider bundleProvider = this.myCodeSystemDaoR4
		...
		return (CodeSystem) new CodeSystem().setUrl(url)
				.setId((IIdType) this.createId(CodeSystem.class, UUID.randomUUID().toString()));
	}
	...
	private void updateCodeSystem(CodeSystem codeSystem, Set<String> codes) {
		...
		this.myCodeSystemDaoR4.update(codeSystem);
	}
