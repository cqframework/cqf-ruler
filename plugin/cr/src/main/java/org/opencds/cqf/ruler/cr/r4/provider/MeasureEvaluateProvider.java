...
public class MeasureEvaluateProvider extends DaoRegistryOperationProvider {
	...
	public MeasureReport evaluateMeasure(RequestDetails requestDetails,
		...
		if (urlVersionManifestMap != null) {
			logger.info("UrlVersionManifestMap : {}", urlVersionManifestMap);
			theMeasure = validateMeasureVersion(theMeasure, urlVersionManifestMap, requestDetails);
		}
		...
		MeasureReport report = measureProcessor.evaluateMeasure(theMeasure.getUrl(), periodStart, periodEnd, reportType,
			subject, null, lastReceivedOn, null, null, null, additionalData);
