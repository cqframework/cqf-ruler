...
import static org.opencds.cqf.ruler.cr.repo.RequestDetailsCloner.startWith;
...
public class RulerRepository implements Repository {
	private static final org.slf4j.Logger ourLog = org.slf4j.LoggerFactory.getLogger(RulerRepository.class);
	private final DaoRegistry myDaoRegistry;
	private final RequestDetails myRequestDetails;
	private final RestfulServer myRestfulServer;

	public RulerRepository(DaoRegistry theDaoRegistry, RequestDetails theRequestDetails,
	...
	private <B extends IBaseBundle> B createBundle(RequestDetails theRequestDetails,
		...
		return (B) BundleProviderUtil.createBundleFromBundleProvider(myRestfulServer,
				theRequestDetails, count, linkSelf, includes, theBundleProvider, start,
				bundleType, thePagingAction);
	}

	// TODO: The main use case for this is paging through Bundles, but I suppose
	// that technically
	// we ought to handle any old link. Maybe this is also an escape hatch for
	// "custom non-FHIR
	// repository action"?
	@Override
	public <B extends IBaseBundle> B link(Class<B> theBundleType, String theUrl,
		...
		if (pageIdParams != null && pageIdParams.length > 0 &&
				isNotBlank(pageIdParams[0])) {
	...
	private void validateHaveBundleProvider(String thePagingAction,
		...
		if (theBundleProvider == null) {
			...
			String msg = fhirContext().getLocalizer().getMessage(RulerRepository.class,
