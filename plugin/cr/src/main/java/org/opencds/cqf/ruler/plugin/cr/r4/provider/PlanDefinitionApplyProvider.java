...
public class PlanDefinitionApplyProvider implements CanonicalUtilities {
    ...
    private Boolean meetsConditions(Session session, PlanDefinition.PlanDefinitionActionComponent action, RequestDetails theRequest) {
        if (action.hasAction()) {
            ...
            if (condition.getKind() == PlanDefinition.ActionConditionKind.APPLICABILITY) {
                ...
                Object result;
					 if (language.equals("text/cql.identifier") || language.equals("text/cql-identifier")
                        ...
                        result = expressionEvaluation.evaluateInContext(session.getPlanDefinition(), cql, session.getPatientId(), theRequest);
							 }
                        ...
                        if (action.getDefinitionCanonicalType().getValue().contains("ActivityDefinition")) {
                            ...
                            try {
                                this.activityDefinitionApplyProvider
                                        .apply(theRequest, new IdType(action.getDefinitionCanonicalType().getId()), patientId, null,
