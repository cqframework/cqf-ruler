...
public class ActivityDefinitionApplyProvider {
	...
	private static final Logger logger = LoggerFactory.getLogger(ActivityDefinitionApplyProvider.class);
	...
	public Resource apply(RequestDetails theRequest, @IdParam IdType theId,
		...
		try {
			...
			return createErrorOutcome("Unable to resolve ActivityDefinition/" + theId.getValueAsString());
		}

		return resolveActivityDefinition(activityDefinition, patientId, practitionerId, organizationId, theRequest);
	}
	...
	public Resource resolveActivityDefinition(ActivityDefinition activityDefinition, String patientId,
		...
		for (ActivityDefinition.ActivityDefinitionDynamicValueComponent dynamicValue : activityDefinition
			...
			// Catch errors while setting each dynamicValue so that entire
			...
			try {
				if (dynamicValue.getExpression() != null) {
					...
					if ("Patient".equals(dynamicValue.getExpression().getExpression())) {
						 ...
						 * resource?
						 */
						Object value = expressionEvaluation.evaluateInContext(activityDefinition,
	...
	private Task resolveTask(ActivityDefinition activityDefinition, String patientId) {
		...
		Extension collectsWith = activityDefinition
				.getExtensionByUrl("http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-collectWith");
