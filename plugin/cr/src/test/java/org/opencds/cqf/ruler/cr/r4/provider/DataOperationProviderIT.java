			...
			switch (dr.getType()) {
				...
				case "Encounter": {
					String query = dr.getExtensionByUrl(
							...
							.getValueAsPrimitive().getValueAsString();
					if (dr.hasCodeFilter()) {
						assertEquals("Encounter?subject=Patient/{{context.patientId}}&type:in=http://cts.nlm.nih.gov/fhir/ValueSet/2.16.840.1.113883.3.117.1.7.1.292", query);
					}
					else {
				...
				case "Coverage": {
					String query = dr.getExtensionByUrl(
							"http://hl7.org/fhir/us/cqfmeasures/StructureDefinition/cqfm-fhirQueryPattern")
							.getValueAsPrimitive().getValueAsString();
					assertEquals("Coverage?policy-holder=Patient/{{context.patientId}}&type:in=http://cts.nlm.nih.gov/fhir/ValueSet/2.16.840.1.114222.4.11.3591", query);
				}
				break;
			}
		}
			...
			switch (dr.getType()) {
				...
				case "Encounter": {
					String query = dr.getExtensionByUrl(
							...
							.getValueAsPrimitive().getValueAsString();
					if (dr.hasCodeFilter()) {
						assertEquals("Encounter?status=finished&subject=Patient/{{context.patientId}}&type:in=http://cts.nlm.nih.gov/fhir/ValueSet/2.16.840.1.113883.3.117.1.7.1.292", query);
					}
					else {
						assertEquals("Encounter?subject=Patient/{{context.patientId}}", query);
					}
			...
			switch (dr.getType()) {
				...
				case "Condition": {
					if (dr.hasCodeFilter() && dr.getCodeFilter().size() > 0) {
						assertEquals("Condition?category:in=http://mcg.com/fhir/ValueSet/condition-problem-list-category&subject=Patient/{{context.patientId}}", query);
					}
					else {
