...
class LibraryEvaluationProviderIT extends RestIntegrationTest {
	...
	void testSimpleLibraryWithBundle() {
		...
		Parameters params = org.opencds.cqf.ruler.utility.r4.Parameters.newParameters(
			...
			org.opencds.cqf.ruler.utility.r4.Parameters.newPart("useServerData", new BooleanType(false))
		);
		Parameters result = getClient().operation()
			.onInstance(new IdType("Library", "SimpleR4Library"))
			.named("$evaluate")
	...
	void testOpioidRec10Library() {
		...
		Parameters params = org.opencds.cqf.ruler.utility.r4.Parameters.newParameters(
			org.opencds.cqf.ruler.utility.r4.Parameters.newPart("subject", new StringType("Patient/example-rec-10-no-screenings"))
		);
		Parameters result = getClient().operation()
		...
		assertTrue(result.hasParameter("Chronic Pain Opioid Analgesic with Ambulatory Misuse Potential Prescriptions"));
		...
		assertTrue(result.hasParameter("Patient Is Being Prescribed Opioid Analgesic with Ambulatory Misuse Potential"));
		assertTrue(((BooleanType) result.getParameter("Patient Is Being Prescribed Opioid Analgesic with Ambulatory Misuse Potential")).booleanValue());
		assertTrue(result.hasParameter("Is Perform Drug Screen Recommendation Applicable?"));
		assertTrue(((BooleanType) result.getParameter("Is Perform Drug Screen Recommendation Applicable?")).booleanValue());
		assertTrue(result.hasParameter("Applicable Because of Positive Cocaine or PCP or Opiates"));
		assertFalse(((BooleanType) result.getParameter("Applicable Because of Positive Cocaine or PCP or Opiates")).booleanValue());
		assertTrue(result.hasParameter("Urine Drug Screening ProcedureRequest Category"));
		assertTrue(result.getParameter("Urine Drug Screening ProcedureRequest Category") instanceof CodeableConcept);
		assertTrue(result.hasParameter("Detail"));
		assertEquals("Patients on opioid therapy should have a urine drug test performed every 12 months.", ((StringType) result.getParameter("Detail")).getValue());
		...
		assertEquals("Annual Urine Screening Check", ((StringType) result.getParameter("Summary")).getValue());
