...
public interface TranslatorUtilities {

	default String errorsToString(Iterable<CqlTranslatorException> exceptions) {
	...
	default CqlTranslator getTranslator(String cql, LibraryManager libraryManager, ModelManager modelManager) {
		return getTranslator(new ByteArrayInputStream(cql.getBytes(StandardCharsets.UTF_8)), libraryManager,
			modelManager);
	}

	default List<CqlTranslator.Options> generateDefaultTranslatorOptions() {
	...
	default CqlTranslator getTranslator(InputStream cqlStream, LibraryManager libraryManager,
	...
	default CqlTranslatorOptions getTranslatorOptions() {
