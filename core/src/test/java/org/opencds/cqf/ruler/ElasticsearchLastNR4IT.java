...
// ElasticsearchLastNR4IT.Initializer.class)
...
// embeddedElastic = new
// ElasticsearchContainer(ELASTIC_IMAGE).withStartupTimeout(Duration.of(300,
// ChronoUnit.SECONDS));
...
// IIdType obsId =
// ourClient.create().resource(obs).execute().getId().toUnqualifiedVersionless();

// myElasticsearchSvc.refreshIndex(ElasticsearchSvcImpl.OBSERVATION_INDEX);

// Parameters output =
// ourClient.operation().onType(Observation.class).named("lastn")
...
// assertEquals(obsId,
// b.getEntry().get(0).getResource().getIdElement().toUnqualifiedVersionless());
// }
...
// the correct port
// TestPropertyValues.of("elasticsearch.rest_url=" + embeddedElastic.getHost()
