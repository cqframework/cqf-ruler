...
import org.opencds.cqf.external.annotations.OnCorsPresent;
import org.opencds.cqf.external.annotations.OnImplementationGuidesPresent;
import org.opencds.cqf.external.common.validation.IRepositoryValidationInterceptorFactory;
import org.opencds.cqf.external.util.EnvironmentHelper;
...
import static org.opencds.cqf.external.common.validation.IRepositoryValidationInterceptorFactory.ENABLE_REPOSITORY_VALIDATING_INTERCEPTOR;
...
public class JpaStarterConfig {
	private static final org.slf4j.Logger ourLog = org.slf4j.LoggerFactory.getLogger(JpaStarterConfig.class);
	@Bean
	...
	@Autowired
	protected IFhirSystemDao fhirSystemDao;
	/**
	...
	public RestfulServer restfulServer(IFhirSystemDao<?, ?> fhirSystemDao, AppProperties appProperties, DaoRegistry daoRegistry, Optional<MdmProviderLoader> mdmProviderProvider, IJpaSystemProvider jpaSystemProvider, ResourceProviderFactory resourceProviderFactory, JpaStorageSettings jpaStorageSettings, ISearchParamRegistry searchParamRegistry, IValidationSupport theValidationSupport, DatabaseBackedPagingProvider databaseBackedPagingProvider, LoggingInterceptor loggingInterceptor, Optional<TerminologyUploaderProvider> terminologyUploaderProvider, Optional<SubscriptionTriggeringProvider> subscriptionTriggeringProvider, Optional<CorsInterceptor> corsInterceptor, IInterceptorBroadcaster interceptorBroadcaster, Optional<BinaryAccessProvider> binaryAccessProvider, BinaryStorageInterceptor binaryStorageInterceptor, IValidatorModule validatorModule, Optional<GraphQLProvider> graphQLProvider, BulkDataExportProvider bulkDataExportProvider, BulkDataImportProvider bulkDataImportProvider, ValueSetOperationProvider theValueSetOperationProvider, ReindexProvider reindexProvider, PartitionManagementProvider partitionManagementProvider, Optional<RepositoryValidatingInterceptor> repositoryValidatingInterceptor, IPackageInstallerSvc packageInstallerSvc, ThreadSafeResourceDeleterSvc theThreadSafeResourceDeleterSvc, ApplicationContext appContext, Optional<IpsOperationProvider> theIpsOperationProvider) {
		...
		ourLog.info("Loading metadata extenders from plugins");
		Map<String, MetadataExtender> extenders = appContext.getBeansOfType(MetadataExtender.class);
		for (MetadataExtender o : extenders.values()) {
			ourLog.info("Found {} extender", o.getClass().getName());
		}

		fhirServer.registerProviders(resourceProviderFactory.createProviders());
		fhirServer.registerProvider(jpaSystemProvider);
		fhirServer.setServerConformanceProvider(calculateConformanceProvider(fhirSystemDao, fhirServer, jpaStorageSettings, searchParamRegistry, theValidationSupport, extenders));
	...
	public static IServerConformanceProvider<?> calculateConformanceProvider(IFhirSystemDao fhirSystemDao, RestfulServer fhirServer, JpaStorageSettings jpaStorageSettings, ISearchParamRegistry searchParamRegistry, IValidationSupport theValidationSupport, Map<String, MetadataExtender> extenders) {
		...
		if (fhirVersion == FhirVersionEnum.DSTU2) {
			...
			ExtensibleJpaConformanceProviderDstu2 confProvider = new ExtensibleJpaConformanceProviderDstu2(fhirServer,
				fhirSystemDao,
				jpaStorageSettings, extenderList);
			...
			ExtensibleJpaCapabilityStatementProvider confProvider = new ExtensibleJpaCapabilityStatementProvider(fhirServer,
				fhirSystemDao,
				jpaStorageSettings, searchParamRegistry, theValidationSupport, extenderList);
			...
			List<MetadataExtender<IBaseConformance>> extenderList = extenders.values().stream()
				.map(x -> (MetadataExtender<IBaseConformance>) x).collect(Collectors.toList());
			ExtensibleJpaCapabilityStatementProvider confProvider = new ExtensibleJpaCapabilityStatementProvider(fhirServer,
				fhirSystemDao, jpaStorageSettings, searchParamRegistry, theValidationSupport, extenderList);
