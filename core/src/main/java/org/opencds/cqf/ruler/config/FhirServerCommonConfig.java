...
import org.opencds.cqf.external.util.JpaHibernatePropertiesProvider;
...
public class FhirServerCommonConfig {

	private static final org.slf4j.Logger ourLog = org.slf4j.LoggerFactory.getLogger(FhirServerCommonConfig.class);


	public FhirServerCommonConfig(AppProperties appProperties) {
	...
	public JpaStorageSettings jpaStorageSettings(AppProperties appProperties) {
		...
		if (appProperties.getAllowed_bundle_types() != null) {
			jpaStorageSettings.setBundleTypesAllowedForStorage(appProperties.getAllowed_bundle_types().stream().map(Bundle.BundleType::toCode).collect(Collectors.toSet()));
		}
	...
	public PartitionSettings partitionSettings(AppProperties appProperties) {
		...
		if (appProperties.getPartitioning() != null) {
			...
			if(appProperties.getPartitioning().getAllow_references_across_partitions()) {
				retVal.setAllowReferencesAcrossPartitions(PartitionSettings.CrossPartitionReferenceMode.ALLOWED_UNQUALIFIED);
			} else {
				retVal.setAllowReferencesAcrossPartitions(PartitionSettings.CrossPartitionReferenceMode.NOT_ALLOWED);
			}
		}
